...
public class ServerHandler implements HttpHandler {
    ...
    private Response doPut(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.getClientMessage(httpExchange.getRequestBody());
            logger.info("Receive updated message with ID " + message.getId());
            if (messageStorage.updateMessage(message)) {
                logger.info("Message has been successfully updated");
                return Response.ok();
            }
            logger.info("Try to remove a non-existent message");
            return Response.badRequest("Try to remove a non-existent message");
        } catch (ParseException e) {
            logger.error("Can not parse message ", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
        }
    }
    ...
    private Response doDelete(HttpExchange httpExchange) {
        String query = httpExchange.getRequestURI().getQuery();
        if (query == null) {
            logger.info("Empty query in request");
            return Response.badRequest("Empty query in request");
        }
        Map<String, String> map = queryToMap(query);
        String messageId = map.get(Constants.REQUEST_PARAM_MESSAGE_ID);
        if (StringUtils.isEmpty(messageId)) {
            logger.info("Try to delete message without id");
            return Response.badRequest("Try to delete message without id");
        }
        if (messageStorage.removeMessage(messageId)){
            logger.info("Message has been successfully removed");
            return Response.ok();
        }
